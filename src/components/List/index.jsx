...
function List({ items, isRemovable, onListClick, onRemove, onClickItem, activeItem }) {
  ...
  return (
    <ul className='list' onClick={onListClick}>
      {items &&
        items.map((item, index) => (
          <li
            ...
            <span>{item.name}</span>
            {isRemovable && (
              <button className='list__remove-btn' onClick={() => removeFolder(item)}>
                <svg
                  width='11'
                  height='11'
                  viewBox='0 0 11 11'
