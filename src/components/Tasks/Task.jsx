...
function Task({ list, task, removeTask, onEditTaskTitle, onCompleteTask }) {
  const editTask = () => {
    const newTaskName = window.prompt('Название задачи', task.text);
    if (newTaskName) {
      onEditTaskTitle(task, newTaskName);
      axios.patch('http://localhost:3001/tasks/' + task.id, { text: newTaskName }).catch(() => {
        alert('Не удалость поменять название задачи');
      });
    }
  };
  ...
  return (
    <div className='tasks__items-row' key={task.id}>
      ...
      <input value={task.text} readOnly></input>
      <div className='tasks__btns'>
        <button className='tasks__edit' onClick={editTask}>
          <svg
            width='15'
            height='15'
            viewBox='0 0 15 15'
        ...
        <button className='tasks__remove' onClick={() => removeTask(task)}>
          <svg
            width='11'
            height='11'
            viewBox='0 0 11 11'
