...
function App() {
  ...
  const onEditTaskTitle = (task, title) => {
    const newLists = lists.map((list) => {
      if (list.id === task.listId) {
        list.tasks.find((item) => item.id === task.id).text = title
      }
      return list
    })
  ...
  const onCompleteTask = (listId, taskId, completed) => {
    const newLists = lists.map((list) => {
      if (list.id === listId) {
        list.tasks.find((item) => item.id === taskId).completed = completed
      }
      return list
    })
  ...
  return (
    <div className="todo">
      <div className="todo__sidebar">
        ...
        <AddButtonList colors={colors} items={lists} onAddLists={onAddLists} />
      </div>
